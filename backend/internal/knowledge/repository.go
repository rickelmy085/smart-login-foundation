...
func (r *DocumentRepo) FindByHash(ctx context.Context, hash string) (string, error) {
	fmt.Printf("[KNOWLEDGE] FindByHash hash=%s\n", hash)
	var id string
	err := r.db.QueryRowContext(ctx, `SELECT id FROM documents WHERE hash = ?`, hash).Scan(&id)
	if err == sql.ErrNoRows {
		fmt.Printf("[KNOWLEDGE] FindByHash hash nao encontrado hash=%s\n", hash)
		return "", nil
	}
	if err != nil {
		fmt.Printf("[KNOWLEDGE] FindByHash erro: %v\n", err)
		return "", err
	}
	fmt.Printf("[KNOWLEDGE] FindByHash sucesso hash=%s id=%s\n", hash, id)
	return id, nil
}
...
func (r *DocumentRepo) InsertDocument(ctx context.Context, d Document) error {
	fmt.Printf("[KNOWLEDGE] InsertDocument id=%s title=%s path=%s\n", d.ID, d.Title, d.SourcePath)
	...
	if err != nil {
		fmt.Printf("[KNOWLEDGE] InsertDocument erro: %v\n", err)
		return err
	}
	fmt.Printf("[KNOWLEDGE] InsertDocument sucesso id=%s\n", d.ID)
	return nil
}
...
func (r *DocumentRepo) DeleteChunks(ctx context.Context, documentID string) error {
	fmt.Printf("[KNOWLEDGE] DeleteChunks documentID=%s\n", documentID)
	_, err := r.db.ExecContext(ctx, `DELETE FROM chunks WHERE document_id = ?`, documentID)
	if err != nil {
		fmt.Printf("[KNOWLEDGE] DeleteChunks erro: %v\n", err)
		return err
	}
	fmt.Printf("[KNOWLEDGE] DeleteChunks sucesso documentID=%s\n", documentID)
	return nil
}
...
func (r *DocumentRepo) InsertChunk(ctx context.Context, documentID string, ord int, content string, start, end int) error {
	fmt.Printf("[KNOWLEDGE] InsertChunk documentID=%s ord=%d start=%d end=%d\n", documentID, ord, start, end)
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO chunks (document_id, ord, content, char_start, char_end)
		VALUES (?, ?, ?, ?, ?)
	`, documentID, ord, content, start, end)
	if err != nil {
		fmt.Printf("[KNOWLEDGE] InsertChunk erro: %v\n", err)
		return err
	}
	return nil
}
...
func (r *DocumentRepo) CountDocuments(ctx context.Context) (int, error) {
	fmt.Println("[KNOWLEDGE] CountDocuments")
	var n int
	err := r.db.QueryRowContext(ctx, `SELECT count(*) FROM documents`).Scan(&n)
	if err != nil {
		fmt.Printf("[KNOWLEDGE] CountDocuments erro: %v\n", err)
		return n, err
	}
	fmt.Printf("[KNOWLEDGE] CountDocuments sucesso count=%d\n", n)
	return n, nil
}

func (r *DocumentRepo) CountChunks(ctx context.Context) (int, error) {
	fmt.Println("[KNOWLEDGE] CountChunks")
	var n int
	err := r.db.QueryRowContext(ctx, `SELECT count(*) FROM chunks`).Scan(&n)
	if err != nil {
		fmt.Printf("[KNOWLEDGE] CountChunks erro: %v\n", err)
		return n, err
	}
	fmt.Printf("[KNOWLEDGE] CountChunks sucesso count=%d\n", n)
	return n, nil
}
